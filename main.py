...
def check_ip_ports(ip, ports, resolve_hostname):
    ...
    if resolve_hostname:
        try:
            hostname, _, _ = gethostbyaddr(str(ip))
            hostname = f"{hostname} - "
        except herror:
            hostname = ""
    for port in ports:
...
def scan_ips():
    start_ip = ".".join([start_ip_entry1.get(), start_ip_entry2.get(
    ), start_ip_entry3.get(), start_ip_entry4.get()])
    end_ip = ".".join([end_ip_entry1.get(), end_ip_entry2.get(),
                      end_ip_entry3.get(), end_ip_entry4.get()])
    ports = list(map(int, ports_entry.get().split(',')))

    start_ip = ipaddress.IPv4Address(start_ip.strip())
    end_ip = ipaddress.IPv4Address(end_ip.strip())

    ips = [ipaddress.IPv4Address(ip_int)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        results = []
        for ip in ips:
            ip_results = list(executor.map(lambda port: check_ip_ports(
                ip, [port], resolve_hostname), ports))
...
def start_scan():
    ...
    try:
        ...
        messagebox.showerror(
            "Error", "Invalid ports. Please enter comma-separated integers.")
        return

    if not is_valid_ports(ports):
        messagebox.showerror(
            "Error", "Invalid ports. Ports must be between 0 and 65535.")
        return
    ...
    try:
        ...
        messagebox.showerror(
            "Error", "Invalid number of threads. Please enter an integer.")
        return

    if not is_valid_threads(max_workers):
        messagebox.showerror(
            "Error", "Invalid number of threads. Threads must be between 1 and 100.")
        return
...
style.configure("TLabel", padding=(50, 5))
...
resolve_hostname_checkbutton = Checkbutton(
    app, text="Hostname", variable=resolve_hostname_var)
resolve_hostname_checkbutton.grid(row=4, column=1, columnspan=3, sticky=W)

Button(app, text="Start Scan", command=start_scan).grid(
